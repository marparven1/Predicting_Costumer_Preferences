...
Hay varias variables importantes de cara a predecir la marca preferida.
...
Las variables más importantes son el salario, la edad y el crédito disponible.
...
Hiperparámetros del mejor modelo: mtry=10
...
# ggplot(ImpGB)
```
En este modelo, las variables más importantes, con grán diferencia son: el salario y la edad.

#### Gráfica del remuestreo

Para examinar la relación entre la estiamción del rendimiento del modelo y los parámetros de ajuste.
...
nrow(DatosTesteoValidacion)-nrow(DatosTesteoValidacion[DatosTesteoValidacion$Marca==PredValC50,])
```
...
```{r}
PredValRandomForest<-predict(BestModelRandomForest, newdata = DatosTesteoValidacion)
#DatosTesteoValidacion[DatosTesteoValidacion$Marca==PredValRandomForest,]
nrow(DatosTesteoValidacion)-nrow(DatosTesteoValidacion[DatosTesteoValidacion$Marca==PredValRandomForest,])
```
...
```{r}
Modelos <- c("C5.0","Random Forest","KNN","Gradient Boosting (Automatic Grid)")
Precision<-c(97.53,88.91,72.22,92.30)
Kappa<-c("-",76.41,40.86,83.69)

cbind.data.frame(Modelos,Precision,Kappa) %>% 
  kable(booktabs=TRUE,caption = "Resultados de los modelos") %>% 
  kable_styling(latex_options = "striped")
```
...
Por tanto, el modelo que nos da las mejore métricas es el modelo C5.0





## Test del modelo final (C5.0)
...
ImpC5.0Test<-cbind.data.frame(
  c("salary","age","Nivel Educación","Marca Coche","Crédito","CodigoPostal"),
  c( 100,84.92,24.41,7.84,6.18,0))
colnames(ImpC5.0Test)=c("Variable","Importancia")
...
```{r}
ggplot(ImpC5.0Test, aes(x =sort( Variable)),) +
...
(MC_C5.0<-matrix(c(855  ,  81   ,  74 , 1464),byrow = TRUE,
                nrow = 2,ncol = 2,
                dimnames =  list(c("Acer","Sony"),c("Acer","Sony"))))
```

```{r}
precisionTest <- (855+1464)/(855+1464+74+81)
cat("La precisión del modelo C5.0 en los datos de testeo es del:", 100* precisionTest)
```


```{r}
modeloC5.0_Test$boostResults
```
Con estos hiperparámetros consigo un 6.3% de error (en los datos de entrenamiento era del 5%)
