...
```
Con estos hiperparámetros consigo un 5.4% de error.
...
(MC_C5.0<-matrix(c(2678,130,144,4472),byrow = TRUE,
                nrow = 2,ncol = 2,
...
ImpC5.0<-cbind.data.frame(
  ...
  c( 100,82.53,38.39,35.94,22.24,21.43	))
...
La siguiente variable con más importancia es la edad (7.4%). Es decir, tenemos unas métricas un poco pobres, y quizás sea porque el modelo depende tanto del salario, que no es capaz de clasificar bien la marca preferida de los clientes.
...
#gbmBestGrid <-  expand.grid(interaction.depth = 9, n.trees = 30, shrinkage = 0.1,n.minobsinnode = 7)
#BestGradientBoostingR <- train(Marca~., data = DatosEntreamiento, method = "gbm", trControl=fitControl, tuneLength = 5, tuneGrid = gbmBestGrid)
...
#Precision<-c("-",89.72,72.27,92.60)
#Kappa<-c("-",78.12,40.88,84.34)
