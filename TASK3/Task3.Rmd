...
Vamos a estudiar como influyen las distintas variables en **el volumen de ventas** de los diferentes productos, con el objetivo de entender como varían las ventas en función del producto.
...
Esto es un problema a la hora de entrenar modelos, porque a la hora de hacer una partición en datos de entrenamiento y testeo, como es probable que tengamos variables con varianzas próximas a 0, esto supondrá errores en los modelos.
...
Var %>% kable(caption = "Varianza de las variables",col.names = "Varianza")
```


Las varianzas de las variables _Product type_ son muy pequeñas, todas son menores que 0,2 y la más pequeña es de 0,012, esto son valores muy bajos para una varianza de cara a modelar.

Además, habíamos visto que prácticamente todas salvo las variables _product type_ tenían una correlación con el volumen de ventas muy baja. Optaremos por eliminar de los datos de cara a elaborar modelo todas las variables product type debido a que nos llevarán a tener peores métricas y más variabilidad en los modelos.
...
### Tipo de producto y volumen ventas (importes generados) quizas me da igual
...
El mayor número de reseñas de 4 estrellas es para los smartphones y con un número de reseñas similar, los ordenadores portátiles. 
...
No ha habido reseñas de dos estrellas en esta variable
...
Al haber unicamente dos modelos, vemos que la recta se ajusta a estos puntos.
...
La pendiente es mayor en la recta de regresión de la variable reseñas positivas por el servicio.
...
Las variables que utilizaremos para predecir el volumen de ventas en el modelo de regresión  son las siguientes:
...
PredictoresRegresion%>% 
  kable(booktabs=TRUE,col.names = "Variables predictoras", caption="Variables para predecir el volumen de ventas con un modelo de regresión") %>% 
  kable_styling(latex_options = "striped") 
```
...
Para el resto de modelos, introduciremos todas las variables disponibles en el el conjunto de datos, que son:

```{r}
Predictores<- c( "Price"                ,"x4StarReviews"               , "x2StarReviews"           ,"PositiveServiceReview"  , "NegativeServiceReview"       ,"Recommendproduct"    , "ShippingWeight"           ,"ProductDepth"                , "ProductWidth"   ,"ProductHeight"                   , "ProfitMargin" 
) 
...
- $R^2_{ajustado}=0.7905$, es decir, las variables dependientes consiguen explicar un 79,05% de la variabilidad del volumen de ventas. No es un mal ajuste
...
En este modelo, la mayoría de variables introducidas influyen fuertemente en la predicción del volumen de ventas. En concreto, las 3 variables que más influyen a la hora de predecir el volumen de ventas son: Reseñas positivas por el servicio, 4 estrellas y medida de profundidad del producto. El resto de variables, salvo la variable 2 estrellas (0,39%) y el precio (8,13%), influyen en el modelo con un porcentaje no muy bajo (mayor al 15%)
...
Observamos que en la primera iteracción el valor de $R^2$ es de 0.818, que es un buen valor (consigo obtenener un 80% de la variabilidad total del modelo), sin embargo, en la segunda iteracción de la primera repetición el valor de $R^2=0,35$, un valor bastante bajo. Hay mucha variabilidad.
...
La variable más importante a la hora de redecir el volumen de ventas es la variable _reseña positiva del servicio_. Del resto de variables, únicamente la variable 4 estrellas participa con un 4%.
...
```
Aqú vemos como la variabilidad en las métricas es bastante menor y por tanto, el modelo será mucho más fiable de cara a predecir.

Los valores de $R^2$ varían entre 0.82 y 0.96, llegando a alcanzar incluso un valor de 0.99.
...
### Algoritmo 4: Gradient Boosting (Automathic Grid)

Este algoritmo no es adecuado con nuestros datos.
...
cbind.data.frame(ProductNumRTesteo,PredRL,DatosTesteoR$Volume) %>% 
  kable(booktabs=TRUE,caption = "Modelo de Regresión Lineal", col.names = c("Producto","Predicción","Valor real")) %>%
  kable_styling(latex_options = "striped")
```
Valores de ventas negativas...

```{r}
plot(DatosTesteoR$Volume,PredRL, 
     main = "Predicciones VS Valor real \n Modelo de regresión lineal",
     xlab="Producto", ylab="Volumen de ventas", yaxt ="n",xaxt = "n",
     xlim = c(0,200), ylim=c(0,2500))
axis(1, at=seq(0, 200, 10))
axis(2, at=seq(0, 3000, 100))
par(new=T)
plot(DatosTesteoR$Volume,DatosTesteo$Volume,
     col="red",xlab = "", ylab = "",yaxt ="n",xaxt = "n")
...
```{r}
BESTmodeloSVM$resample # comentar y mirar a fondo con el que me quede
```
...
## Predicciones de la marca prefererida en el dataset de nuevos productos
...
#   `Rcuadrado`=c(92.27,91.77,70.54,92.38),
#   `RMSE` = c(83.51, 82.39,36.75,83.77)
# ) %>% 
#   kable(booktabs=TRUE) %>% 
#   kable_styling(latex_options = "striped")
```
