...
```{r}
output$PrediccionFinalR<-floor(output$PrediccionFinal)
## output %>% 
##   filter(ProductType=="PC" |ProductType=="Laptop" | ## ProductType=="Netbook" | ProductType=="Smartphone") %>% 
##   select(c(1,2,19)) %>% 
## kable(booktabs=TRUE) %>% 
...
MisProds2 <- MisProds %>%group_by(ProductType) %>% 
  summarise(TotalVentas =  sum(PrediccionFinalR))


ggplot(data=MisProds2, aes(x=ProductType ,   y =TotalVentas , fill= ProductType)) + 
    geom_bar(stat="identity", position="dodge") +
   scale_y_continuous(breaks = seq(0,1500,by=100))+
    labs(title="Gráfico de barras" ,
...
```{r}
MisProds %>% select(c(2,9,5,20)) %>% kable(booktabs=TRUE) %>% kable_styling(latex_options = "striped")


ggplot(data=MisProds, aes(y=PrediccionFinalR,   x =PositiveServiceReview , fill= ProductType,label=ProductNum,colour=ProductType)) + 
    geom_point() +
   scale_x_continuous(breaks=seq(0,40,by=5))+
  scale_y_continuous(breaks = seq(0,1200,by=100))+
    geom_text_repel(box.padding = 0.3, max.overlaps = Inf)+
    labs(title="Reseñas positivas y predicción de ventas" ,
         subtitle = "Según tipo de producto",
         caption="Fuente: Elaboración propia. Conjunto de datos del histórico de ventas",
         xlab="Tipo de producto", ylab="Volumen de ventas histórico")+
  xlab("Número de reseñas positivas por el servicio")+
  ylab("Predicción de ventas")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
ggplot(data=MisProds, aes(x=ProductNum, 
                    y=PrediccionFinalR, fill=ProductType
                    #, colour=ProductNum
                    ) )+ 
    geom_bar(stat="identity", position="dodge")+
  scale_x_continuous(breaks = MisProds$ProductNum)+
 scale_y_continuous(breaks = seq(0,1200,by=100))+
  labs(title = "Predicción de ventas",
       subtitle = "Según tipo de producto",
       caption = "Fuente: Elaboración propia")+
  xlab("Número de identificación del producto")+
  ylab("Predicción de ventas")+
